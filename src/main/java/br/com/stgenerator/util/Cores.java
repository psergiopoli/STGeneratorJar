...
public class Cores {
	...
	private static HashMap<String, ColorName> cores;
	
	private static void initColorsHash(){
		cores = new HashMap<String, ColorName>();
		cores.put("red", new ColorName("red", 0xFF, 0x00, 0x00));
		cores.put("black", new ColorName("black", 0x00, 0x00, 0x00));
	}
	...
	public static Color getColor(String name){
		...
		if(cores.containsKey(name)){
			colorName = cores.get(name);
		}else{
			colorName = cores.get("black");
		}
		Color color = new Color(colorName.getR(),colorName.getG(),colorName.getB());
		return color;
	}
